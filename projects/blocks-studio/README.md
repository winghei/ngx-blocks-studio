...
- **Block loader** — Render components from a JSON description. Supports instance refs (e.g. `FormState.firstName`), two-way binding via refs, nested blocks, and a block registry so blocks can reference each other. Use full descriptions or reuse definitions by `blockId` with overrides.
